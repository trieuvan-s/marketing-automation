...
Phát hiện quan trọng (Phase 0): `SheetsBoard.regroup_and_merge_content()` dùng
Sheets API `mergeCells` (MERGE_COLUMNS) cho cột Context — API này XOÁ THẬT giá
trị mọi ô bị merge trừ ô đầu tiên (không chỉ ẩn hiển thị). Hệ quả: sau 1 lần
merge, `existing_content_keys()` (so khớp theo Context text) không còn thấy
được các dòng video_script/infographic đã merge -> mất idempotency, chính là
cơ chế "content mồ côi". TopicKey PHẢI là cột RIÊNG, KHÔNG nằm trong
`SheetsBoard._CONTENT_MERGE_COLS` -> không bao giờ bị merge xoá.
