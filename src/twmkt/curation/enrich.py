...
def cluster_by_event(items: list[dict], *, threshold: float = 0.6) -> list[dict]:
    """Gộp bài cùng sự kiện (kể cả chéo báo). Mỗi item: dict có title, url,
    publisher, priority (số lớn = ưu tiên cao). Trả về các cụm: bản đại diện là
    báo Priority cao nhất, URL các báo khác gộp vào 'sources'.
    """
    clusters: list[dict] = []
    for it in items:
        placed = False
        for c in clusters:
            if title_similarity(it.get("title", ""), c["rep"].get("title", "")) >= threshold:
                if it.get("priority", 0) > c["rep"].get("priority", 0):
                    c["sources"].append(c["rep"].get("url", ""))  # hạ bản cũ xuống nguồn phụ
                    c["rep"] = it
                else:
                    c["sources"].append(it.get("url", ""))
                placed = True
                break
        if not placed:
            clusters.append({"rep": it, "sources": []})
    for c in clusters:
        c["sources"] = [u for u in c["sources"] if u]
    return clusters
