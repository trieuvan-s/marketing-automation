...
- `src/twmkt/media_factory/spec.py` — **CHỈ CÒN 2 hằng** `VISUAL_KINDS`/`DEFERRED_VISUAL_KINDS` (tập đóng `visual_kind`, dùng bởi `agents/production.py::VideoScriptAgent`). ⚠️ **DỌN DEAD CODE 2026-07-28**: `ProductionSpec`/`ProductionScene`/`verify_spec()` ĐÃ XOÁ — bị THAY THẾ, không phải bỏ tính năng: guardrail-2 VIDEO nay chạy bằng bản PORT TypeScript `aigen-pipeline/src/production-spec/guardrail/verify-spec.ts` (PHA 1.1 đối chiếu tay 100% semantics; chạy thật 2026-07-28 ra `video.mp4`). Dòng cũ ở đây ghi "dùng bởi `aigen_seam.py`" là **SAI** — seam KHÔNG import gì từ file này, nó chỉ truyền path rồi gọi subprocess.
...
- `src/twmkt/media_factory/aigen_seam.py` — seam subprocess gọi `npm run produce:content` (aigen-pipeline) từ Python. **ĐANG DÙNG THẬT từ 2026-07-28**: `scripts/render_production_assets.py::render_video_one()` gọi khi Gate 2 duyệt dòng video. (Trước đó gọi `npm run pipeline` — SAI cổng: cổng đó nhận `TemplateScript` đã chuyển đổi và KHÔNG tự bật OmniVoice TTS.) Ghi chú cũ "cần đánh giá lại còn cần không" đã hết hiệu lực.
